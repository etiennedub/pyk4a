from pyk4a import PyK4APlayback, K4AException, SeekOrigin
import pytest
from pathlib import Path

RECORD_LENGTH=3733444
...
class TestInit:
    ...
    def test_path_argument():
        path_str = 'some.mkv'
        path = Path(path_str)
        playback = PyK4APlayback(path=path)
        assert playback.path == path

        playback = PyK4APlayback(path=path_str)
        assert playback.path == path
...
class TestPropertyCalibrationJson:
    ...
    def test_correct_value(playback: PyK4APlayback):
        playback.open()
        assert playback.calibration_json == RECORD_CALIBRATION_JSON
